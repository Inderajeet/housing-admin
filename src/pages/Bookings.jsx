...
// Unit type colors and labels - Sale includes Plot
...
const Bookings = ({ type = null }) => { // Accept type prop from route
  ...
  // Format date without time for display in table
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      // Pass the route type as filter if not 'all'
  ...
  const handleExport = () => {
    ...
    XLSX.writeFile(wb, `Bookings_${type || 'All'}_${new Date().toLocaleDateString()}.xlsx`);
  };
    ...
    { 
      ...
      accessor: b => {
        const status = BOOKING_STATUSES.find(s => s.value === b.status) || 
          { label: b.status?.toUpperCase() || '-', color: 'bg-gray-100 text-gray-800' };
        return (
  ...
  const getPageTitle = () => {
    if (type === 'rent') return 'Rent Bookings';
    if (type === 'sale') return 'Sale & Plot Bookings';
    return 'All Bookings';
  };

  return (
    <div className="space-y-6">
      {/* Header - Same style as RentProperties */}
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-500 text-xs uppercase tracking-widest font-bold mt-1">
            {type === 'rent' ? 'Manage rental property bookings' : 
             type === 'sale' ? 'Manage sale & plot bookings' : 
      ...
      {/* Data Table - Same style as RentProperties */}
      {loading ? (
        <Loader />
      ) : (
        <div className="overflow-x-auto">
          <DataTable 
            ...
            emptyMessage={`No ${type || ''} bookings found`}
            onRowClick={handleView}
          />
        </div>
      )}

      {/* View Modal - Same style as RentProperties */}
      {isViewOpen && selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4 md:p-10">
          <div className="bg-white rounded-3xl w-full max-w-2xl shadow-2xl overflow-hidden flex flex-col max-h-full">
            ...
            <div className="p-8 overflow-y-auto flex-1 custom-scrollbar space-y-6">
              ...
              <div className="grid grid-cols-2 gap-6">
                ...
                <div className="flex flex-col space-y-1">
                  ...
                  <p className="font-semibold font-mono text-gray-800">
                    {selected.formatted_id || selected.property_id}
                  </p>
                </div>
              </div>
              ...
              {/* Buyer Information - Card style like RentProperties */}
              ...
              <div className="grid grid-cols-2 gap-6">
                <div className="flex flex-col space-y-1">
                  ...
                  <span className={`inline-block px-4 py-2 rounded-xl text-xs font-bold uppercase w-fit ${
                    BOOKING_STATUSES.find(s => s.value === selected.status)?.color || 'bg-gray-100 text-gray-800'
                  }`}>
                ...
                <div className="flex flex-col space-y-1">
                  ...
                  <p className="text-xl font-bold text-gray-900">
                    {formatCurrency(selected.token_amount)}
                  </p>
                </div>
              </div>
            ...
            <div className="px-8 py-6 border-t flex justify-end bg-gray-50 shrink-0">
              <button 
                onClick={() => setIsViewOpen(false)} 
                className="px-6 py-2 rounded-xl border border-gray-300 font-bold text-xs uppercase text-gray-600 hover:bg-gray-100 transition-colors"
              >
                Close
              </button>
            </div>
